...
Version `v3` for @krowdy-ui/pickers is coming with a whole bunch of breaking changes. This is a guide of how-to update :)
<Ad />

#### ⬇️ Install

First of all, you need to install the new package, because the project is transferred to mui-org!
Please also note, that **v3** of `@krowdy-ui/pickers` relies on **v4** of `@krowdy-ui/core`

```
npm i @krowdy-ui/pickers @krowdy-ui/core@latest

yarn add @krowdy-ui/pickers @krowdy-ui/core@latest
```

**Note**: It's also required to upgrade your `@krowdy-ui/core` version to `@next`
...
```diff
- import { InlineDatePicker } from '@krowdy-ui/pickers'
+ import { DatePicker } from '@krowdy-ui/pickers'
